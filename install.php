...
if (!empty($epmdbversion)) {
    ...
    if (version_compare_freepbx($epmdbversion,'1.9','<')) {
		...
		(6, 'update_server', 'http://freepbx.sinfocom.it/provisioner/'),
		(7, 'version', '2.0.0'),
		(8, 'enable_ari', '0'),
		(9, 'debug', '0'),
		(10, 'arp_location', '" . $arp . "'),
    ...
    if (version_compare_freepbx($epmdbversion,'2.10.2.1','<=')) {
        ...
        $sql = "UPDATE endpointman_global_vars SET value = 'http://freepbx.sinfocom.it/provisioner/v3/' WHERE var_name ='update_server'";
        $db->query($sql);
...
if (empty($epmdbversion)) {
  ...
  PRIMARY KEY (`luid`)
    ...
    $sql = "INSERT INTO `endpointman_global_vars` (`idnum`, `var_name`, `value`) VALUES
            ...
            (4, 'gmthr', ''),
            (5, 'config_location', '/tftpboot/'),
            (6, 'update_server', 'http://freepbx.sinfocom.it/provisioner/v3/'),
